...
class Product(db.Model):
    ...
    def until_expire_in_secs(self):
        """returns time until book expires in minutes"""
        expr_date = self.get_expr_date()
        delta = expr_date - datetime.utcnow()
        delta_in_secs = delta.total_seconds()
    ...
    def get_bid_status(self):
        state_min = self.until_expire_in_mins()
        state_sec= self.until_expire_in_secs()
        biddable=True
        if state_min > 0 and state_sec >0:
             #status = "Expires in" + str(round(state_min)) + "minutes" + str(round(state_sec)) + "seconds"
             status = int(round(state_sec))
             #return str(state_min) + ","+str(state_sec);
             return status
        else:
              biddable = False
              status='bidding expired'
              #return status
              return 0
