...
#Emit Background Thread
async_mode = None
#calling Flask Application in localhost
application = Flask(__name__)
# Secret Key is initialized for a flask application to enable session
application.secret_key = "super secret key"
#Debug Error Environment
...
@application.route('/')
def main():
    #return 'Hello Kamalesh from Flask!'
        ...
        try:
            ...
            if name and password:
                    ...
                    if not u.check_password(password):
                        ...
                        db.session.commit()
                        test_message()
                        msg = "Welcome To EAuction  %s ." % user.first_name
                        flash(msg)
...
def Register():
    if request.method == 'POST':
        try:
            ...
            password = request.form['inputPassword']
            if "VoiceEnabled" in request.form:
                voice = request.form['VoiceEnabled']
            else:
                voice=0
            #flash(voice)
            # validate the received values
...
@application.route('/Deactivate', methods=['GET'])
@login_required
def deactivate():
    '''
    This function signs the user out of the system
   ...
   # print(user.get_Voice_Enabled())
    if user_found1 > 1:
       user_found1 = 1
       msg = "User %s Active in a bid." % user1.first_name
       flash(msg)
       return redirect(url_for('products'))
    else:
       # put user_id in session for later use
       ...
       try:
           db.session.commit()
           msg = "%s DeActivated" % user1.first_name
           flash(msg)
       except:
           db.session.rollback()
           #return redirect(url_for('Register'))
           ...
           #msg = "%s DeActivated" % user.first_name
           #flash(msg)
           #return redirect(url_for('main')
           #session.close()
           #msg = "%s DeActivated" % user.first_name
